...
### Episodes for conversation capture

Every customer interaction — chat transcript, phone call summary, email thread — enters the system as an Episode grain (Section 8.2). Episodes capture raw, unstructured text with minimal required fields: `type`, `content`, and `created_at`.
...
### ToolCalls for system interactions

Every API call the agent makes — CRM lookups, ticket updates, payment processing — is recorded as a ToolCall grain (Section 8.5):

```json
{
  "type": "tool_call",
  "tool_name": "crm_lookup_customer",
  "arguments": {"customer_id": "customer-456", "fields": ["subscription", "history"]},
  "result": {"subscription_tier": "enterprise", "tenure_years": 6, "lifetime_value": 28400},
  "success": true,
  "duration_ms": 145,
  "created_at": 1768471200000,
  "namespace": "support:billing",
...
```json
{
  ...
  "arguments": {"customer_id": "customer-456", "amount": 499.00, "invoice": "INV-789", "reason": "erroneous_charge"},
  "result": {"credit_id": "CR-20260115-001", "new_balance": 0.00},
  "success": true,
  "duration_ms": 312,
...
The `success` field, `duration_ms`, and full `arguments`/`result` capture create a complete audit trail. When something goes wrong — a payment fails, a CRM call times out — the `error` field records what happened, and `success: false` flags the failure.

### Checkpoints for multi-session issues

Complex issues that span multiple interactions use Checkpoint grains (Section 8.3) to snapshot the full state:
...
4. **Act**: ToolCall grains record the CRM lookup, the credit application, and the ticket update.

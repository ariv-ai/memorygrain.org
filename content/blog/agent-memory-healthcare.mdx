...
## The seven memory types in clinical context

OMS defines seven memory types (Section 8), each with a type byte in the 9-byte fixed header that enables O(1) filtering without payload deserialization. In a healthcare system, every type has a natural clinical role.
...
### Episodes for clinical encounters

Episode grains (type byte `0x02`) store raw, unstructured text — perfect for clinical notes and consultation transcripts:
...
Episodes are the input to consolidation — the process of extracting structured Facts from unstructured text. A clinical AI agent might process this Episode and generate Fact grains: `(patient-456, has_symptom, persistent_cough)`, `(patient-456, prescribed, codeine_cough_suppressant)`. The extracted Facts would carry `source_type: "llm_generated"` and reference the Episode in their `derived_from` field.
...
### ToolCalls for lab orders and prescriptions

Every interaction with external clinical systems — lab order submissions, prescription fills, EHR API queries — becomes a ToolCall grain (type byte `0x05`):
...
The `duration_ms` field tracks API response times — useful for monitoring EHR integration performance. The `success` boolean and optional `error` field create an automatic record when orders fail, enabling systematic analysis of integration failures.

### Checkpoints for care plan state

Checkpoint grains (type byte `0x03`) capture the complete state of a care plan at a point in time:
...
1. **Patient check-in**: The clinical agent creates a Checkpoint grain capturing current care plan state
...
3. **Clinical encounter**: The consultation is recorded as an Episode grain with the raw clinical notes
4. **Diagnosis update**: The agent generates or confirms Fact grains with `relation: "diagnosed_with"`, linking to the Observation and Episode grains via `derived_from`
5. **Lab order**: The agent calls the lab order API, creating a ToolCall grain with the order details and result
...
This is not a hypothetical architecture. It is a direct application of the seven memory types, the sensitivity classification system, the per-user encryption pattern, the selective disclosure mechanism, and the bi-temporal model — all defined in the OMS v1.0 specification with precise binary formats and validation rules.

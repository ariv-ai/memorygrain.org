...
Goal state transitions follow the immutable supersession model. When the "Pick part A" goal is completed, a new grain is created with `goal_state: "satisfied"` and `satisfaction_evidence` referencing the Observation or ToolCall grains that confirm success. The original active goal gets `superseded_by` set in the index layer, preserving the complete state transition history.
...
On recovery, the robot loads the most recent Checkpoint grain, restores its state from the `context` map, and resumes from the current step in the `plan`. Because Checkpoints are immutable and content-addressed, the recovery process is deterministic — the same checkpoint always produces the same restored state.
...
4. **Checkpoint grains** save state at each major phase transition — after navigation, after grasp, after placement — enabling recovery if anything goes wrong.

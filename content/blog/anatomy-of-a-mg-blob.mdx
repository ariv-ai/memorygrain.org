...
The smallest valid blob is a 9-byte header followed by `0x80` -- an empty MessagePack fixmap (a map with zero entries). This is technically valid at the format level, though it would fail schema validation for any of the seven memory types (all require at least `type` and `created_at`).

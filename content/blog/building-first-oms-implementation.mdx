...
You understand the concepts — content addressing, canonical serialization, field compaction, the seven memory types. Now you want to build something. Where do you start?
...
4. **Support all 7 memory types** — Fact, Episode, Checkpoint, Workflow, ToolCall, Observation, and Goal. Your deserializer must handle the required fields for each.
...
4. Extract the type byte. Values `0x01` through `0x07` are the seven standard memory types.
...
After decompaction, check that the `type` field exists. If absent, reject with `ERR_NO_TYPE`. The value must be one of: `"fact"`, `"episode"`, `"checkpoint"`, `"workflow"`, `"tool_call"`, `"observation"`, or `"goal"`.

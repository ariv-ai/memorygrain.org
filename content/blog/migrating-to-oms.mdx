...
Each of these maps to one or more of the seven OMS memory types. The mapping is not arbitrary --- it follows directly from the type definitions in Section 8 of the specification.
...
### Conversation logs become Episode grains

Raw conversation transcripts are Episodes --- the simplest grain type. Section 8.2 requires just three fields: `type`, `content`, and `created_at`.
...
Episodes are intentionally unstructured. They are the input to consolidation --- the process by which an agent extracts structured Facts from raw interactions. The optional `consolidated` boolean tracks whether an episode has been processed. You can migrate entire conversation histories as Episodes and then run consolidation to produce Facts, preserving the full provenance chain via `derived_from`.

### API call logs become ToolCall grains

Every tool invocation your agent makes --- API calls, function calls, database queries --- maps to a ToolCall grain. Section 8.5 captures the complete audit record:
...
### Configuration snapshots become Checkpoint grains
...
- Recognize all seven memory types
...
Using the mappings described above, assign each data source to one or more OMS types. A single source might produce multiple grain types --- a conversation log produces Episode grains directly and, after consolidation, Fact grains indirectly.

...
Every grain carries a `type` field indicating what kind of knowledge it represents. OMS v1.0 defines ten standard grain types, each with its own required and optional fields. The type is also encoded in byte 2 of the fixed header, enabling O(1) filtering without deserializing the payload.
